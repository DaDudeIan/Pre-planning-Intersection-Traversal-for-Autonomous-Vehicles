...
To create a baseline with which the topology-based loss functions can be compared, the first results to be presented are those from the models trained purely with the cross-entropy loss. This serves as a baseline since CE is a well-documented, well-tested loss function that will help highlight whether or not the topology-based loss function are necessary. Results from different stages of training are shown in #subfigure("Figure 26-28"), with outputs being generated from the models after the #nth(10), #nth(100), and #nth(300) epochs, respectively. 
...
The input image to the models in this section comes from the training set. Again, the figures shows the outputs of the models after the #nth(10), #nth(100), and #nth(300) epochs. The results are shown in #subfigure("Figure 30-32"). The models have been trained on this data, so it is expected that they perform well on it.
