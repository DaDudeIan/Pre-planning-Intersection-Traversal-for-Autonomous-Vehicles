...
#std-block(breakable: false,
  figure(
    ...
    caption: [Results after the #nth(10) epoch. Already the loss has a noticeable impact on the output structure, particularly for the convolution-based models.]
  )
)
...
#std-block(breakable: false,
  figure(
    ...
    caption: [Results after the #nth(50) epoch. Particularly DeepLab is able to learn the structure of the intersection with this loss function. The apparent "shell" of wrongly predicted pixels surrounding the predicted path, stems from the fact that the loss does not enforce any class structure. ]
  )
)
