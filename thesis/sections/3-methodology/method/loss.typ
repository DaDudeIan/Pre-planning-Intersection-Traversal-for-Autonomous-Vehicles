...
#text("CONSIDER DOING MEAN AKIN TO BCE", weight: "bold", fill: red)
...
where $C$ is the cold map vector and $P$ is the predicted path vector, giving a scalar value, contributing to the total loss. The implementation of @eq:loss_cold_flat is very straightforward, and is shown in the code listing below:

#listing([
  ```python
def cmap_loss(cmap_gt, path_pred) -> torch.Tensor:
...
#std-block(breakable: false)[
  ...
  #figure(
  image("../../../figures/img/loss_example/cmap_loss_comparison3.png", width: 100%),
