...
@code.sat shows two functions, `get_sat_image` and `save_sat_image`, that are used to retrieve and save satellite imagery, respectively. The `get_sat_image` function first checks if a `secret` and coordinates are provided as they are required. It then constructs a URL for the Google Maps Static #acr("API") request and signs it using the `sign_url` function detailed in @alg.url_signing. The signed URL is then used to make a request to the #acr("API"), and the response containing the image is returned. This response can then be passed to the `save_sat_image` function, which saves the image to a file with the specified filename.
