...
Furthermore, it should be noted that other DL methodologies were considered. First, is reinforcement learning, which was not chosen as it would add unnecessary complexity to the project. Second, is the use of generative models, which were also not chosen as they introduce a massive change in the training paradigm. Both of these points highlight the fact that their implementation would require a significant paradigm shift in this project, which was deemed to go beyond the scope of this thesis. 
...
$
  theta_(t+1,i) = theta_(t,i) - eta (1/sqrt(hat(v)_t + epsilon) dot hat(m)_t + w_(t,i)theta_(t,i)), forall t 
$
