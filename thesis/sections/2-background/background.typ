...
The first commercially available #acr("ACC") vehicles came from Chrysler in 1958 following the invention of the SpeedoStat @first_cc. They named this system the "Auto-Pilot" and vehicles equipped with it were some of the first vehicles in history to achieve what would later be called Level 1 autonomy. Soon after, Cadillac adopted the technology for their own vehicles and dubbed it "Cruise Control", which since became the default term for the ACC technology, which is still used today, though usually with the term "adaptive" prepended.
...
#h(4mm) The image classification task is the simplest of the computer vision tasks. It is concerned with classifying an image into one of a number of classes. This task, which was AlexNet's main task, is shown in @fig:cv_tasks#subfigure("a"). As shown, it is concerned with labelling the entire image as one class. It should be noted "class" means the name of a group of similar things, like the example shows the class of the image is "tiger". This idea of classes, becomes important when moving on to the other CV tasks. The next task is object detection, which is concerned with detecting and localizing objects in an image. This task is shown in @fig:cv_tasks#subfigure("b"). It is concerned with not only classifying the image, but also localizing the objects in the image. This is done by drawing bounding boxes around the objects in the image, and classifying them. Note that there may be multiple instances of the same objects in the image, each requiring their own bounding box for correct classification.
...
#let c = [*Patch embedding*: An input image is divided into equally-sized non-overlapping patches, typically 16x16 pixels each. These patches are flattened into vectors and transformed into embeddings through a #text("learned linear projection", fill: rgb(252, 225, 224).darken(20%), stroke: 0.1pt + rgb(252, 225, 224).darken(40%)), producing fixed-dimensional patch embeddings. *Transformer encoder*: The embeddings go through multiple stacked layers of #text("transformer encoder blocks", fill: rgb(229, 229, 229).darken(20%), stroke: 0.1pt + rgb(229, 229, 229).darken(40%)), each comprising two sub-layers: a #text("multi-headed self-attention mechanism", fill: rgb(199, 232, 172).darken(20%)) and a #text("feed-forward neural network", fill: rgb(193, 228, 247).darken(20%), stroke: 0.1pt + rgb(193, 228, 247).darken(10%)). Multi-headed self-attention computes attention scores to weigh the relevance of each patch relative to all others simultaneously. To retain spatial information, #text("positional embeddings", fill: rgb(209, 188, 210).darken(20%)) are added to the patch embeddings before being input into the transformer layers. *Classification head*: The processed embeddings from the transformer layers include a special classification token (`[class]`), prepended to the sequence of patch embeddings. The `[class]` embedding captures a global representation of the image. This global embedding is then passed through a #text("fully-connected neural network", fill: rgb(255, 210, 164).darken(20%)) to produce class predictions for image classification tasks.]

#wrap-content(fig, c, align: right, columns: (1fr, 2fr))

#h(4mm) Closely related to the ViT is the Swin Transformer, introduced by Liu #etal @swin in 2021. The Swin Transformer is a hierarchical transformer that uses a shifted windowing scheme to reduce the computational cost of self-attention. This is done by dividing the input image into non-overlapping windows, and then applying self-attention within each window. These windows are merged in the deeper layers of the network, achieving great highly accurate segmentation masks. 
...
For AVs, the datasets are often more complex than just images. They often consist of multiple sensors, such as LiDAR, radar, and cameras. This balloons the size of the dataset, resulting in singular entries being multiple gigabytes in size. The Waymo Open Dataset @waymo_dataset is a prime example of this. It consists of 1950 entries, each consisting of 20 seconds of video at 10Hz. Each contains the data from a mid-range LiDAR, 4 short-range LiDARs, and 5 cameras. The entries contain labelled data for 4 object classes: vehicles, pedestrians, cyclists, and signs. @fig:datasets#subfigure("e") shows an example of some LiDAR readings overlaid on the corresponding camera image. Waymo is a very detailed dataset, with a lot of data to learn from. However, it is also a very large dataset, which makes it difficult to work with on consumer-level computers. Competitors do exist, such as the nuScenes @nuscenes dataset and the Argoverse @argoverse dataset. 

Closely related are the lane-level datasets. These datasets are concerned with detecting and classifying lanes in images. This is a very important task for AVs, as it allows them to understand the road topology and navigate accordingly. The TuSimple dataset example shown in @fig:datasets#subfigure("d") illustrates this task well, providing annotated lane markings for training and evaluation. For systems like #acr("AIM") that manage the vehicle going through an intersection, datasets like the INTERACTION @interactiondataset dataset are needed. This dataset consists of overhead views of intersections, with annotated vehicles moving through them. This dataset is very useful for training models to understand the interactions between vehicles at intersections. 

Finally, some satellite image-based datasets also exists for different purposes. The SpaceNet @spacenet dataset is concerned with the labelling of buildings and roads etc. in satellite images. This dataset consists of high-resolution satellite images and different annotations for the images, such as individual houses, both through bounding boxes and segmentation masks. For a bit wider scale, the DeepGlobe @deepglobe dataset offers semantic masks of roads, building, and land cover.
  ...
  #figure(
    grid(
      ...
      grid( columns: (1.11fr, 1fr), 
        fig5, fig4, 
        [#subfigure("(a)") SpaceNet @spacenet], [#subfigure("(b)") INTERACTION @interactiondataset]),
      grid( columns: (1fr, 0.73fr, 1.09fr), 
      fig2, fig3, fig1, 
      [#subfigure("(c)") DeepGlobe @deepglobe], [#subfigure("(d)") TuSimple @tusimple], [#subfigure("(e)") Waymo @waymo_dataset])
    ),
